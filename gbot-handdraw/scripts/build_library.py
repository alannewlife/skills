...
.masthead{{min-height:220px;padding:30px clamp(20px,6vw,90px) 26px;display:grid;grid-template-columns:minmax(0,1fr);gap:28px;align-items:end;border-bottom:1px solid var(--line);background:linear-gradient(115deg,#f8f2e5 0 72%,#d8d1bb 72%)}}
...
.intro{{max-width:680px;margin:14px 0 0;font-size:15px;line-height:1.55;color:#4f5048}}
...
@media(max-width:700px){{.masthead{{min-height:190px;padding:24px 20px 22px;grid-template-columns:1fr;background:#f8f2e5}}h1{{font-size:40px}}.toolbar{{position:relative}}.filters{{order:3;width:100%;overflow-x:auto}}.result-count{{margin-left:0}}.collection{{padding-top:46px}}.chapter{{gap:16px}}.chapter-letter{{width:56px;height:56px;font-size:34px;box-shadow:5px 5px 0 var(--vermillion)}}.gallery{{grid-template-columns:repeat(2,minmax(0,1fr));gap:10px}}}}
@media(prefers-reduced-motion:reduce){{*{{scroll-behavior:auto!important;transition:none!important}}}}
</style></head>
<body><header class="masthead"><div><p class="eyebrow">Hand-drawn style field guide · {total} specimens</p><h1>手绘风格标本册 <i>· {total} 种风格</i></h1><p class="intro">每一种画风都有自己的编号。搜索、筛选、查看提示词，然后把编号和你的主题交给 Skill。</p></div></header>
