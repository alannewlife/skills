...
for (let i = 0; i < slideCount; i++) {
  ...
  // Most gbot-frontend-slides presentations use a currentSlide index and show/hide
  await page.evaluate((index) => {
    const slides = document.querySelectorAll('.slide');

    // Try multiple navigation strategies used by gbot-frontend-slides:
  ...
  // Force the current slide into its fully-settled visual state before capture.
  // This handles decks that use reveal variants and delayed CSS transitions.
  await page.evaluate((index) => {
    const slides = document.querySelectorAll('.slide');
    const currentSlide = slides[index];
    const deckAnchor = document.getElementById('deckAnchor');
    const deckPageNumber = document.getElementById('deckPageNumber');
    const navDots = document.getElementById('navDots');

    if (currentSlide) {
      currentSlide.classList.add('active', 'visible');

      if (deckAnchor) {
        const hideAnchor = currentSlide.dataset.hideAnchor === 'true';
        deckAnchor.textContent = currentSlide.dataset.anchor || '';
        deckAnchor.classList.toggle('hidden', hideAnchor);
        deckAnchor.style.opacity = hideAnchor ? '0' : '1';
        deckAnchor.style.visibility = hideAnchor ? 'hidden' : 'visible';
      }

      if (deckPageNumber) {
        deckPageNumber.textContent = `${String(index + 1).padStart(2, '0')} / ${String(slides.length).padStart(2, '0')}`;
        deckPageNumber.style.opacity = '1';
        deckPageNumber.style.visibility = 'visible';
      }

      if (navDots) {
        navDots.style.display = 'none';
        navDots.style.visibility = 'hidden';
      }

      currentSlide.querySelectorAll('.reveal, .reveal-left, .reveal-scale, .reveal-blur').forEach(el => {
        el.style.opacity = '1';
      ...
      currentSlide.querySelectorAll('*').forEach(el => {
  ...
  // Give the browser a brief settle window after disabling transitions.
